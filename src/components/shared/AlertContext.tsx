...
export function AlertProvider({ children }: { children: ReactNode }) {
  ...
  const addAlert = (alertData: Omit<SOSAlert, 'id' | 'timestamp'>) => {
    ...
    // Announce new alert for screen readers
    setAriaAnnouncement(
      `New ${newAlert.severity} severity ${newAlert.type} alert at ${newAlert.institution}, ${newAlert.district}. ${newAlert.description}`
    );
  };
  ...
  return (
    <AlertContext.Provider value={{
      ...
      {/* ARIA Live Region for alert announcements */}
      <AriaLiveRegion 
